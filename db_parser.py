...
def db_parser(filename):
    ...
    try:
        ...
        # Print the resulting dictionary for verification, including the offsets
        for chord, fingers in chord_dict.items():
            print(f"{chord.ljust(12)} : {fingers}")
    except FileNotFoundError:
        print(f"File '{filename}' not found.")
        exit(1)

db_parser('GuitarDict.csv')